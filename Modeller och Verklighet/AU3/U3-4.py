...
def beräknaeffekt(phi, n, t, I_0, alt_p, azimut_p, epsilon, A):
    ...
    I_p = I * (np.cos(np.radians(alt_p - solalt)) * np.cos(np.radians(azimut_p - azimut)) + (1 - np.cos(np.radians(azimut_p - azimut))) * np.sin(np.radians(alt_p)) * np.sin(np.radians(solalt)))
    P = epsilon * I_p * A 
    P = P if P >= 0 else 0
    return P
